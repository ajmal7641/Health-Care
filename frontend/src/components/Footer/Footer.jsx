...
const socialLinks = [
  ...
  {
  path: "https://github.com/ajmal7641",
      ...
      <div className="flex justify-between flex-col md:flex-row flex-wrap gap-[30px]">
        <div>
          ...
          <p className='text-[16px] leading-7 font-[400] text-textColor mt-4'> 
            Copyright © {year} developed by Ajmal k s all right reserved.
            </p>
            <div className='flex items-center gap-3 mt-4'>
              {socialLinks.map((link,index)=> (
              <Link to={link.path} key={index} 
              className='w-9 h-9 border border-solid border-[#181A1E]
              rounded-full flex justify-center items-center group hover:bg-primaryColor hover:border-none'>
                {link.icon}
                </Link>
              ))}
            </div>
